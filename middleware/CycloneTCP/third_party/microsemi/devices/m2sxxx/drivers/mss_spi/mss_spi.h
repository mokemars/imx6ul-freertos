 ...
 *
 * SVN $Revision: 5516 $
 * SVN $Date: 2013-03-29 22:22:18 +0000 (Fri, 29 Mar 2013) $
 */
/*=========================================================================*//**
  ...
  Configuration
  A MSS SPI block can operate either as a master or slave SPI device. There are
  ...
  SPI slave device the MSS SPI block will communicate with. It is used to
  ...
  A call is then made to function MSS_SPI_transfer_frame() specifying and the
  value of the data frame to be sent.
  ...
  MSS_SPI_set_slave_select(). This causes the relevant slave select line to
  become asserted while data is clocked out onto the SPI data line.
  A call is then made to function MSS_SPI_transfer_block (). The parameters of
  ...
  subsequent SPI transactions. A call to this function is only required if the
  master is communicating with multiple slave devices.
  Note: When performing block transfers, the MSS SPI asserts the target slave
        device’s chip select throughout the transfer in Motorola SPI modes 1 and
        3 (i.e. protocol mode configuration is MSS_SPI_MODE1 or MSS_SPI_MODE3),
        whereas the slave select is de-asserted between each byte transfer in
        Motorola SPI modes 0 and 2 (i.e. protocol mode configuration is
  ...
  slave for block transfer operations. It specifies:
    ...
    - The handler function that will be called after the receive buffer is
      filled
  ...
  parameters to function MSS_SPI_set_cmd_handler(). The command handler function
  ...
  specified though the call to MSS_SPI_set_slave_block_buffers() would also
  ...
  call to function MSS_SPI_set_transfer_byte_count(). The DMA transfer is then
  initiated by a call to the MSS_PDMA_start() function provided by the MSS PDMA
  driver. The actual DMA transfer will only start once the MSS SPI block has
  been re-enabled through a call to  MSS_SPI_enable(). The completion of the DMA
...
typedef struct __mss_spi_instance_t
{
    ...
    volatile uint32_t slave_tx_size;    /*!< Size of slave transmit buffer. */
    volatile uint32_t slave_tx_idx;     /*!< Current index into slave transmit buffer. */
    ...
    volatile uint32_t slave_rx_size;    /*!< Slave receive buffer size. */
    volatile uint32_t slave_rx_idx;     /*!< Current index into slave receive buffer. */
...
/***************************************************************************//**
  ...
  communication with the slave. These configuration settings must be specified
  in a previous call to the MSS_SPI_configure_master_mode() function.
